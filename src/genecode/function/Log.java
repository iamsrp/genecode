...
/**
 * A function which computes the natural log of a value.
 */
public class Log
    extends Function
{
     ...
     *             types are supported.
     */
    public Log(final Class<? extends Number> type)
    {
    ...
    protected Object safeCall(final Object[] args)
    {
        ...
        if (getReturnType().equals(Float.class)) {
            return Float.valueOf((float)Math.log(((Number)value).doubleValue()));
        }
        else if (getReturnType().equals(Double.class)) {
            return Double.valueOf(Math.log(((Number)value).doubleValue()));
        }
        else {
            return null;
        }
